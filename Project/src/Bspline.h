...
public:
    ...
    Spline complete_cubic_Bspline() {
        ...
        for (int i = 1; i < N - 1; ++i) {
            coeff_matrix[(i - 1)*N + i] = basis_polynomials[i + 1][n][0](knots[i + 2].first);
            coeff_matrix[i*N + i] = basis_polynomials[i + 2][n][1](knots[i + 2].first);
            coeff_matrix[i*N + i + 1] = basis_polynomials[i + 3][n][2](knots[i + 2].first);
        }
    ...
    Spline arbitrary_order_Bspline() {
