...
public:
    Bspline(int nn, int NN, std::vector<std::pair<double, double>> knots_in, std::vector<double> bc = {}): n(nn), N(NN), knots(knots_in) {
        if (!bc.empty()) {
    ...
    Spline linear_Bspline() {
        ...
        polynomials.push_back(Polynomial(0, std::vector<double>{0.0}));
        polynomials.push_back(Polynomial(0, std::vector<double>{0.0}));
        for (int i = 1; i <= N; ++i) {
    ...
    Spline cardinal_quad_Bspline() {
        ...
        for (int i = 0; i < N; ++i)
            knots[i].first -= 0.5;
        knots.push_back(std::make_pair(knots[N - 1].first + 1, 0.0));
        for (int i = 1; i <= n; ++i) {
            knots.insert(knots.begin(), std::make_pair(knots[0].first - 1, 0.0));
            knots.push_back(std::make_pair(knots[N - 1 + i].first + i, 0.0));
        }

        basis_polynomials.reserve(N + 10);
        for (auto& second_element: basis_polynomials) 
            second_element.reserve(n + 4);
        ...
        for (int i = 1; i <= N + n - 1; ++i) {
            if (i == 1) { 
                double co = 2 * boundary_conditions[0] - solution[0];
                Polynomial temp(0, std::vector<double>{co});
                for (int j = 0; j <= n; ++j)
                    polynomials.push_back(temp * basis_polynomials[i][n][j]);
                continue;
            }
            if (i == N + 1) {
                double co = 2 * boundary_conditions[1] - solution[N - 1];
                Polynomial temp(0, std::vector<double>{co});
                polynomials[N + 1] = polynomials[N + 1] + temp * basis_polynomials[i][n][0];
                continue;
            }
            Polynomial temp(0, std::vector<double>{solution[i - 2]});
            for (int j = 0; j < n; ++j)
                polynomials[i + j] = polynomials[i + j] + temp * basis_polynomials[i][n][j];
            polynomials.push_back(temp * basis_polynomials[i][n][n]); 
        }
        for (int i = 0; i <= n; ++i)
            polynomials.erase(polynomials.begin());
        for (int i = 0; i < n; ++i)
            knots.erase(knots.begin());
        return Spline(N + 1, knots, polynomials);
    }
    ...
    Spline complete_cubic_Bspline() {
        ...
        coeff_matrix[0] = 1.0/3 + (knots[5].first - knots[3].first) / (2 * (knots[5].first + knots[4].first - 2 * knots[3].first));
        coeff_matrix[N] = (knots[4].first - knots[3].first) / (knots[5].first + knots[4].first - 2 * knots[2].first);
        ...
        coeff_matrix[N*N - N - 1] = (knots[N + 2].first - knots[N + 1].first) / (2 * knots[N + 2].first - knots[N + 2].first - knots[N - 1].first);
        coeff_matrix[N*N - 1] = 1.0/3 + (knots[N + 2].first - knots[N - 1].first) / (2 * (2 * knots[N + 2].first - knots[N + 1].first - knots[N].first));

        double rhs[N];

        rhs[0] = knots[3].second + boundary_conditions[0] * (knots[4].first - knots[3].first) / 3;
        for (int i = 1; i < N - 1; ++i)
            rhs[i] = knots[i + 3].second;
        rhs[N - 1] = knots[N + 2].second - boundary_conditions[1] * (knots[N + 2].first - knots[N + 1].first) / 3;

        EquationSolver linear_system(N, coeff_matrix, rhs);
        double *solution = linear_system.solve();
        //for (int i = 0; i < N; ++i)
         //   std::cout << solution[i] << " " << rhs[i] << std::endl;

        std::vector<Polynomial> polynomials;
        polynomials.push_back(Polynomial(0, {0.0}));
        for (int i = 1; i <= N + 2; ++i) {
            if (i == 1) {
                double co = 3 * (knots[4].first - knots[3].first) * solution[1] / (knots[5].first + knots[4].first - 2 * knots[3].first) - 2 * (knots[4].first - knots[3].first) * boundary_conditions[0];
    ...
    Spline natural_cubic_Bspline() {
        ...
        coeff_matrix[N*N - 1] = 1.0/3 + (knots[N + 2].first - knots[N].first) / (2 * (2 * knots[N + 2].first - knots[N + 1].first - knots[N].first));
        ...
        for (int i = 1; i <= N + 2; ++i) {
            if (i == 1) {
                double co = -3 * (knots[4].first - knots[3].first) * solution[1] / (knots[5].first + knots[4].first - 2 * knots[3].first);
                Polynomial temp(0, {co});
                for (int j = 0; j <= n; ++j)
                    polynomials.push_back(temp * basis_polynomials[i][n][j]);
                continue;
    ...
    Spline arbitrary_order_Bspline(double a[]) {
        ...
        for (int i = 0; i < n; ++i)
            knots.erase(knots.begin());
        for (int i = 0; i < N; ++i)
            knots[i].second = polynomials[(i == N - 1) ? i - 1 : i](knots[i].first);
        return Spline(N, knots, polynomials);
    }
